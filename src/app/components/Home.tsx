    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        ...
        if (entry.isIntersecting) {
          ...
          section.querySelectorAll('.hero-name').forEach(el => gsap.set(el, { y: 50, opacity: 0 }));
          section.querySelectorAll('.hero-surname').forEach(el => gsap.set(el, { y: 50, opacity: 0 }));
          section.querySelectorAll('.hero-title').forEach(el => gsap.set(el, { y: 50, opacity: 0 }));
          section.querySelectorAll('.hero-buttons').forEach(el => gsap.set(el, { y: 30, opacity: 0 }));
          section.querySelectorAll('.hero-social a').forEach(el => gsap.set(el, { y: 20, opacity: 0 }));
        }
      },
      { threshold: 0.3 }
    );
  ...
  return (
    <main ref={heroRef}>
      <section className="min-h-screen flex flex-col justify-center items-center text-center px-4 relative overflow-hidden">
        ...
        <h1 className="hero-surname text-5xl md:text-7xl font-bold mb-4">Sadikaj</h1>
        <h3 className="hero-title text-2xl md:text-4xl mb-8">Web Developer</h3>
